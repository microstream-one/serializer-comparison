...
public class SizeRun1
{
    ...
    private static long serializeWithTiming(final List<Product> allProducts, final SerializerFoundation<?> foundation) throws Exception
    {
        // Timings cannot be trusted since JVM performance optimizations.
        final long start = System.nanoTime();
        serialize(allProducts, foundation);
    ...
    private static byte[] serialize(final List<Product> allProducts, final SerializerFoundation<?> foundation) throws Exception
    {
        final byte[] result;
        try (final Serializer<byte[]> serializer = Serializer.Bytes(foundation))
        {

            result = serializer.serialize(allProducts);
        }
        return result;
    }
